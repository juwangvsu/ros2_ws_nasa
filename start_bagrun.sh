#!/bin/bash
gnome-terminal -x  $SHELL -ic "cd ~/; ros2 bag play msbuild/ --clock -r 0.2; bash"
...
gnome-terminal -x  $SHELL -ic "ros2 run pointcloud_to_laserscan_logged pointcloud_to_laserscan_logged_node  --ros-args   --params-file pointcloud_to_laserscan_logged.yaml   -r cloud_in:=/baal/lidar_points   -r scan:=/scan; bash"
