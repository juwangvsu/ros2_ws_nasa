...
# (1) fiducial node (2) gazebo (3) slam (4) rviz2 (5) nav2 (6) go (7) xbox tele (8) pointcloud_to_laserscan_logged_node_depthcloud

gnome-terminal -x  $SHELL -ic "conda deactivate; export ROS_DOMAIN_ID=3; ros2 launch fiducial_tb3_gazebo_demo sim_mapping_anchor.launch.py use_sim_time:=true apriltag_cfg:=apriltag_30.yaml; bash"
...
gnome-terminal -x  $SHELL -ic "conda deactivate; export ROS_DOMAIN_ID=3; ros2 launch slam_toolbox online_async_launch.py slam_params_file:=mapper_params_online_async.yaml use_sim_time:=True"
...
gnome-terminal -x  $SHELL -ic "cd ~/ros2_ws_nasa; export ROS_DOMAIN_ID=3; ros2 run pointcloud_to_laserscan_logged pointcloud_to_laserscan_logged_node_depthcloud  --ros-args   --params-file pointcloud_to_laserscan_camcloud.yaml   -r cloud_in:=/camera/points   -r scan:=/scan2"
