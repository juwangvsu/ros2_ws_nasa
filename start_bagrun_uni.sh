...
gnome-terminal -x  $SHELL -ic "cd ~/; ros2 bag play $1 --topics /unilidar/cloud /unilidar/imu --remap /unilidar/cloud:=/unilidar/cloud --clock -r 0.2; bash"
...
gnome-terminal -x  $SHELL -ic "ros2 run laser_filters scan_to_scan_filter_chain --ros-args --params-file scan_filter.yaml -r scan:=/scan -r scan_filtered:=/scan_filtered" # max_height=2 if too high ceiling points in scan. /scan -> /scan_filtered

gnome-terminal -x  $SHELL -ic "ros2 launch pointlio_tf_bridge pointlio_tf_bridge.launch.py use_sim_time:=true static_pitch:=0.0 static_yaw:=-2.53 bridge_params_file:=bridge.yaml; bash" # 15 deg 0.26, 20 deg 0.348 , 30 deg 0.52 base_link -> baal/imu_initial  bridge.yaml base_link to baal/imu_initial to correct the tf from point_lio
