...
xterm -e bash -ic "cd /tmp; ros2 bag play bag_panda1 --topics /unilidar/cloud /unilidar/imu --remap /unilidar/cloud:=/unilidar/cloud_raw --clock -r 0.2; bash"&
xterm -e bash -ic "python3 change_frame.py; bash"&
xterm -e bash -ic  "ros2 launch point_lio mapping_unilidar_l2.launch.py use_sim_time:=true; bash"&
